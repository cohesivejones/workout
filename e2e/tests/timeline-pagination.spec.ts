...
test.describe('Timeline Pagination', () => {
  ...
  test('user paginates data in list view', async ({ page, request }) => {
    ...
    // Step 2: Create workouts spanning 6+ months
    // We'll create workouts at different dates to test pagination
    const workoutDates = [
      // Recent workouts (within 3 months)
      { daysAgo: 10 },
      { daysAgo: 45 },
      { daysAgo: 80 },
      // Older workouts (4-6 months ago)
      { daysAgo: 120 },
      { daysAgo: 150 },
      { daysAgo: 180 },
    ];
