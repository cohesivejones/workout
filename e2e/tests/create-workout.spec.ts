...
test.describe('Create Workout with Multiple Exercises', () => {
  ...
  test('should create a workout with 6 exercises and verify in list and calendar views', async ({ page }) => {
    // Step 1: Login
    await page.goto('/');
    ...
    for (const exercise of exercises) {
      ...
      await expect(page.locator(`text=${exercise.name} - ${exercise.reps} reps`)).toBeVisible({ timeout: 3000 });
    }
