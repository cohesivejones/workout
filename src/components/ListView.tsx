...
import { toPainScoreNewPath, toPainScoreEditPath, toWorkoutEditPath, toWorkoutPath, toWorkoutNewPath } from "../utils/paths";
...
export const ListView = ({
  ...
  const [isDeleting, setIsDeleting] = React.useState<{ type: string; id: number } | null>(null);

  // Combine workouts and pain scores into a single array
  const items: ListItem[] = [
    ...workouts.map(workout => ({ type: 'workout' as const, data: workout })),
    ...painScores.map(painScore => ({ type: 'painScore' as const, data: painScore }))
  ];
            ...
            if (item.type === 'workout') {
              ...
              return (
                ...
                >
                  <div className="list-card-header">
                    <h3>
                      {format(`${painScore.date}T12:00:00.000`, "MMM d, yyyy (eeee)")}
                    </h3>
                    <div className="list-card-type">Pain Score</div>
                    <div className="list-card-actions">
                      <Link to={toPainScoreEditPath(painScore)} className="edit-btn">
                        Edit
                      </Link>
                      <button
                        onClick={() => handleDeletePainScore(painScore.id)}
                        disabled={isDeleting?.type === 'painScore' && isDeleting.id === painScore.id}
                        className="delete-btn"
                      >
                  ...
                  <div className="list-card-content">
                    <div className="pain-score-level">
                      <strong>Pain Level: {painScore.score}</strong> - {getPainScoreDescription(painScore.score)}
                    </div>
                    {painScore.notes && (
