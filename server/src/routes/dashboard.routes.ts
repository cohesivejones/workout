...
router.get("/weight-progression", authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const exerciseProgressionData: Record<string, Array<{ date: string; weight: number | null; reps: number; new_reps?: boolean; new_weight?: boolean }>> = {};
    ...
    workouts.forEach((workout) => {
      workout.workoutExercises.forEach((we) => {
        ...
        exerciseProgressionData[exerciseName].push({ date: workout.date, weight: we.weight, reps: we.reps, new_reps: we.new_reps, new_weight: we.new_weight });
      });
    });

    const result = Object.entries(exerciseProgressionData).map(([name, dataPoints]) => ({ exerciseName: name, dataPoints }));
    res.json(result);
  } catch (err) {
...
router.get("/pain-progression", authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const startDateStr = startDate.toISOString().split("T")[0];
    const endDateStr = endDate.toISOString().split("T")[0];
    const painScores = await dataSource.getRepository(PainScore).find({ where: { userId: Number(userId), date: Between(startDateStr, endDateStr) }, order: { date: "ASC" } });
...
router.get("/sleep-progression", authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const startDateStr = startDate.toISOString().split("T")[0];
    const endDateStr = endDate.toISOString().split("T")[0];
    const sleepScores = await dataSource.getRepository(SleepScore).find({ where: { userId: Number(userId), date: Between(startDateStr, endDateStr) }, order: { date: "ASC" } });
